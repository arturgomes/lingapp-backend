...
export default {
  ...
  async show(request: Request, response: Response) {
    ...
    const aluno = await alunosRepository.findOneOrFail(id,{ relations: ['turma'] });
    // const aluno2 = await alunosRepository.findOneOrFail(id,{select:['turma_id'], relations: ['turma'] });
    const aluno1 = await alunosRepository.findOne({select:['turma as turma_id'],relations:['turma']}) ;
    console.log(aluno1)
